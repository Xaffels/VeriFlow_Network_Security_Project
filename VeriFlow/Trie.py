...
class Trie(object):
	...
	def generateECs(self):
		self.ecs.clear()
		self.generateECs(self.root, "")
		return self.ecs
	
	def generateECs(self, node, path):

		if(node.numberOfNonStarChildren() == 2):
			ecLeft = self.generateECs(node.getLeftChild(), path + "0")
			ecRight = self.generateECs(node.getRightChild(), path + "1")
			if (node.getLeftChild().isOpenRight() or node.getRightChild().isOpenLeft()):
				self.ecs.add(Interval(ecLeft.getRight(), ecRight.getLeft()))
			if (node.getStarChild() != None):
				if (node.getLeftChild().isOpenLeft()):
					self.ecs.add(Interval(path, ecLeft.getLeft()))
				if (node.getRightChild().isOpenRight()):
					self.ecs.add(Interval(ecRight.getRight(), self.binaryAdd(path, 1)))
				node.setAdvertisedInterval(Interval(path, self.binaryAdd(path, 1)))
			...
			if (node.getLeftChild() != None):
				ecLeft = self.generateECs(node.getLeftChild(), path + "0")
				if (node.getStarChild() != None):
					self.ecs.add(Interval(ecLeft.getRight(), self.binaryAdd(path, 1)))
					if (node.getLeftChild().isOpenLeft()):
						self.ecs.add(Interval(path, ecLeft.getLeft()))
					node.setAdvertisedInterval(Interval(path, self.binaryAdd(path, 1)))
					return node.getAdvertisedInterval()
				...
				ecRight = self.generateECs(node.getRightChild(), path + "1")
				if (node.getStarChild() != None):
					self.ecs.add(Interval(path, ecRight.getLeft()))
					if (node.getRightChild().isOpenRight()):
						self.ecs.add(Interval(ecRight.getRight(), self.binaryAdd(path, 1)))
					node.setAdvertisedInterval(Interval(path, self.binaryAdd(path, 1)))
					return node.getAdvertisedInterval()
